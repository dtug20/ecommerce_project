...
const subjects = {
  en: (d) => `Order Confirmation — #${d.orderNumber}`,
  vi: (d) => `Xác nhận đơn hàng — #${d.orderNumber}`,
};
...
const bodies = {
  en: (d) => layout(
    ...
    `      <h2>Thank you for your order!</h2>
      ...
      <div class="info-row"><span class="label">Order Date:</span> ${d.orderDate}</div>
      <div class="info-row"><span class="label">Order Total:</span> ${d.orderTotal}</div>
      <div class="info-row"><span class="label">Shipping To:</span> ${d.shippingAddress}</div>
      <hr class="divider" />
  ...
  vi: (d) => layout(
    ...
    `      <h2>Cảm ơn bạn đã đặt hàng!</h2>
      ...
      <div class="info-row"><span class="label">Ngày đặt hàng:</span> ${d.orderDate}</div>
      <div class="info-row"><span class="label">Tổng tiền:</span> ${d.orderTotal}</div>
      <div class="info-row"><span class="label">Địa chỉ giao hàng:</span> ${d.shippingAddress}</div>
      <hr class="divider" />
