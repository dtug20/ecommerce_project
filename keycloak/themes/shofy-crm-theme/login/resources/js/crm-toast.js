(function () {
  ...
  var TITLES = window.CRM_TOAST_TITLES || {
    error: 'Error', warning: 'Warning', success: 'Success', info: 'Notice',
  };

  function getContainer() {
    var el = document.getElementById('crm-toast-container');
    if (!el) {
      el = document.createElement('div');
      el.id = 'crm-toast-container';
  ...
  function dismiss(toast) {
    toast.classList.add('crm-toast-out');
    toast.addEventListener('animationend', function () {
      if (toast.parentNode) toast.parentNode.removeChild(toast);
    }, { once: true });
  }

  window.crmToast = function (type, message) {
    ...
    toast.innerHTML =
      '<span class="crm-toast-icon">' + (ICONS[type] || ICONS.info) + '</span>' +
      '<div class="crm-toast-body">' +
        '<div class="crm-toast-title">' + TITLES[type] + '</div>' +
        '<div class="crm-toast-message">' + message + '</div>' +
      '</div>' +
      '<button class="crm-toast-close" aria-label="Close">' +
    ...
    toast.querySelector('.crm-toast-close').addEventListener('click', function () {
      clearTimeout(timer);
      dismiss(toast);
    });
  };
  ...
  document.addEventListener('DOMContentLoaded', function () {
    var alerts = document.querySelectorAll('.crm-alert');
    alerts.forEach(function (el) {
      ...
      if (text) window.crmToast(type, text);
    });
  });
})();
